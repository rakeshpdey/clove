...
use rayon::prelude::*; 
use tokio::io::{AsyncReadExt, AsyncWriteExt}; // Async Network Streams
...
pub struct MultiGPUOrchestrator {
    ...
    // --- ADVANCED: True Ring-AllReduce Network Topology (NCCL Architecture) ---
    pub rank: usize,
    pub world_size: usize,
    // In a Ring Topology, we strictly hold one connection from the left, and one to the right.
    pub rx_stream: Option<Arc<tokio::sync::Mutex<tokio::net::TcpStream>>>, // Incoming from Left
    pub tx_stream: Option<Arc<tokio::sync::Mutex<tokio::net::TcpStream>>>, // Outgoing to Right
...
impl MultiGPUOrchestrator {
    ...
    pub async fn new() -> Self {
        ...
        for adapter in adapters {
            ...
            println!("[ORCHESTRATOR] Found Capable GPU Shard {}: {:?}", shard_id, adapter.get_info().name);
        ...
        println!("[ORCHESTRATOR] Successfully initialized {} Local GPU Shards.", shards.len());
    ...
    pub async fn connect_cluster(&mut self, my_rank: usize, all_addrs: Vec<String>) {
        ...
        // 1. Open a listener for the Left-Neighbor
        let listener = tokio::net::TcpListener::bind(my_addr).await
            .expect("FATAL: Failed to bind to local port! Is the port already in use?");

        // 2. Dial the Right-Neighbor
        ...
        for attempt in 1..=30 {
            if let Ok(stream) = tokio::net::TcpStream::connect(next_addr).await {
                println!("[NETWORK] Rank {} successfully connected to Right-Neighbor!", my_rank);
                tx_stream_opt = Some(stream);
                break;
            }
            println!("[NETWORK] Waiting for Right-Neighbor to come online... (Attempt {}/30)", attempt);
            tokio::time::sleep(std::time::Duration::from_millis(1000)).await;
        }
        let tx_stream = tx_stream_opt.expect("FATAL: Failed to connect to the next node in the ring!");

        // 3. Accept connection from the Left-Neighbor
        let (rx_stream, prev_addr) = listener.accept().await
            .expect("FATAL: Failed to accept connection from Left-Neighbor!");
        
        println!("[NETWORK] Rank {} accepted connection from Left-Neighbor at {}!", my_rank, prev_addr);
    ...
    pub fn distribute_batch(&self, total_batch_size: usize) -> Vec<usize> {
        ...
        if num_shards == 0 { return vec![total_batch_size]; } 
        let base_size = total_batch_size / num_shards;
        let remainder = total_batch_size % num_shards;
        (0..num_shards).map(|i| if i < remainder { base_size + 1 } else { base_size }).collect()
    }
    ...
    pub fn synchronize_gradients(&self, buffers: &[Arc<wgpu::Buffer>]) {
        ...
        // STEP 1: PULL LOCAL (Read data from all local GPU VRAMs into CPU RAM)
        for (i, shard) in self.shards.iter().enumerate() {
            let staging_buf = shard.device.create_buffer(&wgpu::BufferDescriptor {
                label: Some("AllReduce Staging Read"), size: buffer_size, usage: wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false,
            });

            let mut encoder = shard.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
            ...
            shard.device.poll(wgpu::PollType::wait_indefinitely()).unwrap();
        ...
        // STEP 2: AVERAGE LOCAL (Use Rayon to multi-thread the local CPU math)
        let mut local_averaged = vec![0.0f32; num_elements];
        let local_scale = 1.0 / (num_shards as f32);
        
        local_averaged.par_iter_mut().enumerate().for_each(|(idx, avg_val)| {
            let mut sum = 0.0;
            for shard in all_data.iter().take(num_shards) { sum += shard[idx]; }
            *avg_val = sum * local_scale;
        });

        // =====================================================================
        // STEP 3: THE GLOBAL TCP/IP RING-ALLREDUCE (True NCCL Architecture!)
        // =====================================================================
        let global_averaged = if self.world_size > 1 {
            ...
            self.rt.block_on(async {
                ...
                for _ in 0..(self.world_size - 1) {
                    ...
                    tokio::try_join!(send_future, recv_future).expect("FATAL: Ring connection broken during All-Reduce!");
                    
                    // Add the data we just received to our global total
                    global_data.par_iter_mut().zip(recv_buffer.par_iter()).for_each(|(g, r)| {
                        *g += r;
                    });
        ...
        // STEP 4: PUSH (Blast the perfect global gradients back to every local GPU!)
