...
//! Foreign Function Interface (FFI) for the Organon Engine.
//!
//! This module exposes a stable C-ABI, enabling the Organon engine to be
//! embedded into other language runtimes such as Python (via `ctypes` or `cffi`),
//! C++, or Go. It manages the lifecycle of shared pointers and translates
//! between Rust-safe types and raw C-compliant memory structures.
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn organon_tensor_create(
    data: *const c_float,
    rows: usize,
    cols: usize,
) -> CTensor {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn organon_tensor_free(tensor_ptr: CTensor) {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn organon_tensor_data(tensor_ptr: CTensor, out_data: *mut c_float) {
...
// CORE OPERATIONS (Exposed to foreign languages e.g.python,C++)
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn organon_matmul(a_ptr: CTensor, b_ptr: CTensor) -> CTensor {
...
/// Both `a_ptr` and `b_ptr` must be valid `CTensor` pointers created by this library.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn organon_add(a_ptr: CTensor, b_ptr: CTensor) -> CTensor {
    let a = unsafe { &*(a_ptr as *const TensorNode<WgpuBackend>) };
    let b = unsafe { &*(b_ptr as *const TensorNode<WgpuBackend>) };
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn organon_relu(a_ptr: CTensor) -> CTensor {
    let a = unsafe { &*(a_ptr as *const TensorNode<WgpuBackend>) };
    let result = WgpuBackend::relu(a);
    Box::into_raw(Box::new(result)) as CTensor
}

// AUTOGRAD (Exposed to other languages)
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn organon_backward(tensor_ptr: CTensor) {
    if tensor_ptr.is_null() {
        return;
    }
    let tensor = unsafe { &*(tensor_ptr as *const TensorNode<WgpuBackend>) };
