...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn organon_tensor_create(data: *const c_float, rows: usize, cols: usize) -> CTensor {
    let slice = unsafe { std::slice::from_raw_parts(data, rows * cols) };
    let array = Array2::from_shape_vec((rows, cols), slice.to_vec()).unwrap();
    
    // Create the tensor node
    let tensor = WgpuBackend::new(array);
    
    // Move it to the heap and leak the pointer so Rust doesn't drop it. 
    // The foreign language (Python/C++) now owns this memory.
...
/// or operation function within this library. It must not be double-freed.
#[unsafe(no_mangle)]
