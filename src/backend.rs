...
/// Symbolic Integer representation for dynamic tensor shapes.
/// Allows the JIT compiler to trace the graph without knowing exact batch sizes
/// or sequence lengths until execution time.
#[derive(Debug, Clone)]
pub enum SymInt {
    Const(usize),
    Symbol(String),
...
impl SymInt {
    /// Evaluates the symbolic shape into a concrete integer using the runtime environment.
    ...
    /// Converts the symbolic math into WGSL shader code for dynamic GPU dispatches.
    ...
    /// Helper to multiply all dimensions to find the total tensor size.
    pub fn multiply_all(shape: &[SymInt]) -> SymInt {
        shape
            .iter()
            .cloned()
            .reduce(|a, b| SymInt::Mul(Box::new(a), Box::new(b)))
            .unwrap_or(SymInt::Const(1))
    }
}

// AMP (AUTOMATIC MIXED PRECISION)
/// Defines the mathematical precision for tensor operations.
/// Used to trigger hardware-accelerated F16/BF16 Tensor Cores.
...
/// The set of all supported mathematical operations in the framework.
/// Acts as the opcode dictionary for the JIT compiler.
#[derive(Debug, Clone)]
pub enum Opcode {
    Input,
    MatMul,
...
/// The global computation graph used to trace operations before JIT compilation.
...
impl ComputeGraph {
    ...
    /// Pushes a new operation onto the tracing graph.
...
/// Represents the physical or virtual location of tensor data.
pub enum TensorData {
    Cpu(Vec<f32>),
    Gpu(wgpu::Buffer),
    Lazy(usize),
...
/// The core trait defining hardware-agnostic tensor execution.
pub trait Backend: Sized + Send + Sync + 'static {
    ...
    fn layer_norm(
        a: &TensorNode<Self>,
        gamma: &TensorNode<Self>,
        beta: &TensorNode<Self>,
    ) -> TensorNode<Self>;
    ...
    fn batch_norm(
        ...
        gamma: &TensorNode<Self>,
        beta: &TensorNode<Self>,
        r_mean: &TensorNode<Self>,
        r_var: &TensorNode<Self>,
        momentum: f32,
    ) -> TensorNode<Self>;
...
/// The Eager Execution Backend.
/// Directly executes commands on the CPU via Rayon or GPU via WebGPU API.
...
impl WgpuBackend {
    ...
    /// Hardware-accelerated Tiled Matrix Multiplication (WebGPU)
    /// NOTE: Uses L1 shared `var<workgroup>` caching for massive VRAM bandwidth savings.
    ...
    /// Generic executor for element-wise GPU operations.
    #[allow(clippy::too_many_arguments)]
    ...
    /// Backpropagation shader for elementwise addition.
    /// NOTE: Features a Parallel Reduction in L1 memory before falling back to global atomics
    /// to avoid GPU warp divergence during gradient accumulation.
    #[allow(clippy::too_many_arguments)]
    pub fn wgpu_add_grad(
        ...
        let shader_src = "
            ...
            var<workgroup> local_grad: array<f32, 256>;
            ...
            fn main(@builtin(global_invocation_id) global_id: vec3<u32>, @builtin(local_invocation_id) local_id: vec3<u32>) {
                ...
                // Parallel Reduction in fast L1 Shared Memory
                for (var offset = 128u; offset > 0u; offset = offset / 2u) {
                    if (lid < offset) {
                        local_grad[lid] = local_grad[lid] + local_grad[lid + offset];
                    }
                    workgroupBarrier();
                }

                // Only ONE thread per workgroup hits the global atomic CAS loop!
                if (lid == 0u && idx < dims.a_size) {
                    atomicAddFloat(idx % dims.b_size, local_grad[0]);
                }
            }
        ";
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: grad_a.as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
                    resource: grad_b.as_entire_binding(),
                },
            ],
        });
        let mut encoder =
    ...
    /// Backpropagation shader for matrix multiplication.
    #[allow(clippy::too_many_arguments)]
    pub fn wgpu_matmul_grad(
        device: &wgpu::Device,
        queue: &wgpu::Queue,
        grad_out: &wgpu::Buffer,
        a_buf: &wgpu::Buffer,
            ...
            device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        {
            ...
            cpass.set_bind_group(0, &bind_group, &[]);
            cpass.set_pipeline(&pipeline_a);
            cpass.dispatch_workgroups(k.div_ceil(8), m.div_ceil(8), 1);
            cpass.set_pipeline(&pipeline_b);
            cpass.dispatch_workgroups(n.div_ceil(8), k.div_ceil(8), 1);
        }
        queue.submit(Some(encoder.finish()));
    }
}
...
impl Backend for WgpuBackend {
    ...
    fn ones(size: usize, device: &Self::Device) -> Self::TensorPrimitive {
        if let Some((d, q)) = device.get_gpu() {
            ...
            q.write_buffer(&buf, 0, bytemuck::cast_slice(&ones));
            TensorData::Gpu(buf)
        } else {
            ...
            TensorData::Gpu(src_buffer) => {
                if let Some((device, queue)) = device_info.get_gpu() {
                    ...
                    let dst_buffer = device.create_buffer(&wgpu::BufferDescriptor {
                        label: Some("Gradient Checkpoint Clone"),
                        size,
                        usage: wgpu::BufferUsages::STORAGE
                            | wgpu::BufferUsages::COPY_DST
                            | wgpu::BufferUsages::COPY_SRC,
                    ...
                    panic!("AUTOGRAD FATAL: Cannot clone GPU buffer without WGPU device context.");
                }
            }
            TensorData::Lazy(_) => {
                panic!("AUTOGRAD FATAL: Cannot clone Lazy evaluation paths yet.")
            }
        }
    }
    ...
    fn add_cpu_grad(tensor: &mut TensorGraph<Self>, new_grad: &[f32]) {
        if tensor.grad.is_none() {
            tensor.grad = Some(TensorData::Cpu(new_grad.to_vec()));
        } else if let Some(TensorData::Cpu(current)) = tensor.grad.as_mut() {
            current
                .par_iter_mut()
                .zip(new_grad.par_iter())
                .for_each(|(c, &g)| *c += g);
        } else {
            panic!("Hardware Mismatch!");
        }
    }

    fn clip_gradients(tensor: &mut TensorGraph<Self>) {
        if let Some(TensorData::Cpu(ref mut gradients)) = tensor.grad {
            gradients.par_iter_mut().for_each(|g| {
                *g = g.clamp(-1.0, 1.0);
            });
        }
    }
    ...
    fn grad_to_cpu(tensor: &TensorGraph<Self>) -> Option<Array2<f32>> {
        ...
        match tensor.grad.as_ref()? {
            TensorData::Cpu(vec) => {
                Some(Array2::from_shape_vec((rows, cols), vec.clone()).unwrap())
            }
            TensorData::Gpu(buffer) => {
                if let Some((device, queue)) = tensor.device.get_gpu() {
                    let size = buffer.size();
                ...
                if precision == Precision::BF16 || precision == Precision::F16 {
                    if let Some((device, queue)) = a.device.get_gpu() {
                        ...
                        TensorData::Gpu(Self::wgpu_elementwise(
                            &device, &queue, a_buf, a_buf, a_size, a_size, shader,
                        ))
                    } else {
                        unreachable!()
                    }
                    ...
                    panic!(
                        "Hardware F16 execution is routed through precision config, not eager cast."
                    )
                }
            }
    ...
    fn all_reduce(node: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let out_data = match &n_read.device {
            EngineDevice::MultiGpu {
                ...
                println!(
                    "Executing Ring-AllReduce across {} GPU shards...",
                    peers.len() + 1
                );
    ...
    fn matmul(a_node: &TensorNode<Self>, b_node: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let out_data = match (&a.data, &b.data) {
            (TensorData::Cpu(a_vec), TensorData::Cpu(b_vec)) => {
                TensorData::Cpu(Self::rayon_matmul(a_vec, b_vec, m, k, n))
            }
            ...
            backward: Some(Box::new(|out_tensor: &TensorGraph<Self>| {
                ...
                match out_tensor.grad.as_ref().unwrap() {
                    TensorData::Cpu(out_grad) => {
                        ...
                        if let (TensorData::Cpu(a_data), TensorData::Cpu(b_data)) =
                            (&a_read.data, &b_read.data)
                        {
                            a_grad_calc
                                .par_chunks_mut(k)
                                .enumerate()
                                .for_each(|(r, row)| {
                                    for c in 0..k {
                                        let mut sum = 0.0;
                                        for i in 0..n {
                                            sum += out_grad[r * n + i] * b_data[c * n + i];
                                        }
                                        row[c] = sum;
                                    }
                                });
                            b_grad_calc
                                ...
                                .for_each(|(r, row)| {
                                    for c in 0..n {
                                        let mut sum = 0.0;
                                        for i in 0..m {
                                            sum += a_data[i * k + r] * out_grad[i * n + c];
                                        }
                                        row[c] = sum;
                                    }
                                });
    ...
    fn concat_seq(a_node: &TensorNode<Self>, b_node: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let a_shape = if is_3d {
            a.shape.clone()
        } else {
            vec![1, a.shape[0], a.shape[1]]
        };
        let b_shape = if b.shape.len() == 3 {
            b.shape.clone()
        } else {
        ...
        for batch in 0..b_sz {
            let a_start = batch * s1 * h;
            let a_end = a_start + (s1 * h);
            out_vec.extend_from_slice(&a_vec[a_start..a_end]);
            let b_start = batch * s2 * h;
            let b_end = b_start + (s2 * h);
            out_vec.extend_from_slice(&b_vec[b_start..b_end]);
        }
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Cpu(out_vec),
            shape: out_shape,
            grad: None,
            creators: vec![],
        ...
        let out_data = match (&a.data, &b.data) {
            ...
            (TensorData::Gpu(a_buf), TensorData::Gpu(b_buf)) => {
                if let Some((device, queue)) = a.device.get_gpu() {
                    ...
                    ";
                    TensorData::Gpu(Self::wgpu_elementwise(
                        &device, &queue, a_buf, b_buf, a_size, b_size, shader_src,
                    ))
                } else {
                    unreachable!()
                }
            ...
            backward: Some(Box::new(|out_tensor: &TensorGraph<Self>| {
                ...
                match out_tensor.grad.as_ref().unwrap() {
                    ...
                    TensorData::Gpu(out_grad_buf) => {
                        if let Some((device, queue)) = out_tensor.device.get_gpu() {
                            let a_size =
                                a_node.read().unwrap().shape.iter().product::<usize>() as u32;
                            let b_size =
                                b_node.read().unwrap().shape.iter().product::<usize>() as u32;
                            let init_gpu_grad = |node: &TensorNode<Self>, size: u32| {
    ...
    fn mul(a_node: &TensorNode<Self>, b_node: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            device: a.device.clone(),
            backward: Some(Box::new(|out_tensor: &TensorGraph<Self>| {
                // NOTE: Backward logic for Mul
                let a_node = &out_tensor.creators[0];
                let b_node = &out_tensor.creators[1];
                let out_grad = out_tensor.get_cpu_grad();
                let a_read = a_node.read().unwrap();
                let b_read = b_node.read().unwrap();
                let a_len = a_read.shape.iter().product();
                let b_len = b_read.shape.iter().product();
                let mut a_grad_calc = vec![0.0; a_len];
                let mut b_grad_calc = vec![0.0; b_len];
                    ...
                    (&a_read.data, &b_read.data)
                {
                    ...
                    for i in 0..out_grad.len() {
                        ...
                        if b_is_scalar {
                            b_grad_calc[0] += out_grad[i] * a_val;
                        } else {
                            b_grad_calc[i] += out_grad[i] * a_val;
                        }
                    }
                }
                drop(a_read);
        ...
        let out_data = match (&a.data, &b.data) {
            ...
            (TensorData::Gpu(a_buf), TensorData::Gpu(b_buf)) => {
                if let Some((device, queue)) = a.device.get_gpu() {
                    ...
                    ";
                    TensorData::Gpu(Self::wgpu_elementwise(
                        &device, &queue, a_buf, b_buf, a_size, b_size, shader,
                    ))
                } else {
                    unreachable!()
                }
            ...
            _ => {
                let (a_cpu, _) = a.to_cpu().into_raw_vec_and_offset();
                let (b_cpu, _) = b.to_cpu().into_raw_vec_and_offset();
                let mut result = vec![0.0; a_cpu.len()];
                let b_len = b_cpu.len();
                result.par_iter_mut().enumerate().for_each(|(i, res)| {
                    *res = a_cpu[i] - b_cpu[i % b_len];
                });
                TensorData::Cpu(result)
            }
        };
    ...
    fn relu(node: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out_tensor: &TensorGraph<Self>| {
                ...
                match out_tensor.grad.as_ref().unwrap() {
                    TensorData::Cpu(out_grad) => {
                        let out_data = if let TensorData::Cpu(d) = &out_tensor.data {
                            d
                        } else {
                            unreachable!()
                        };
                        let mut p_grad = vec![0.0; out_grad.len()];
                        for i in 0..out_grad.len() {
                            p_grad[i] = if out_data[i] > 0.0 { out_grad[i] } else { 0.0 };
                        }
                        parent.write().unwrap().add_cpu_grad(&p_grad);
                    }
                    TensorData::Gpu(out_grad_buf) => {
                        if let Some((device, queue)) = out_tensor.device.get_gpu() {
                            ...
                            let p_buf = if let TensorData::Gpu(b) = &p_read.data {
                                b
                            } else {
                                unreachable!()
                            };
                            ...
                            ";
                            let grad_calc_buf = Self::wgpu_elementwise(
                                &device,
                                &queue,
                                p_buf,
                                out_grad_buf,
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out_tensor: &TensorGraph<Self>| {
                ...
                for i in 0..out_grad.len() {
                    ...
                    let derivative = 0.5 * (1.0 + tanh_inner.tanh())
                        + 0.5 * x * sech2 * c * (1.0 + 3.0 * 0.044715 * x.powi(2));
    ...
    fn sin(node: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out_tensor: &TensorGraph<Self>| {
                ...
                let mut p_grad = vec![0.0; out_grad.len()];
                for i in 0..out_grad.len() {
                    p_grad[i] = out_grad[i] * p_data[i].cos();
                }
                drop(p_read);
                parent.write().unwrap().add_cpu_grad(&p_grad);
            })),
    ...
    fn cos(node: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out_tensor: &TensorGraph<Self>| {
                ...
                let mut p_grad = vec![0.0; out_grad.len()];
                for i in 0..out_grad.len() {
                    p_grad[i] = out_grad[i] * -p_data[i].sin();
                }
                drop(p_read);
                parent.write().unwrap().add_cpu_grad(&p_grad);
            })),
    ...
    fn dropout(node: &TensorNode<Self>, rate: f32) -> TensorNode<Self> {
        ...
        let out_data = match &a.data {
            ...
            TensorData::Gpu(a_buf) => {
                if let Some((device, queue)) = a.device.get_gpu() {
                    ...
                    TensorData::Gpu(Self::wgpu_elementwise(
                        &device, &queue, a_buf, a_buf, a_size, seed, &shader,
                    ))
                } else {
                    unreachable!()
                }
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
                ...
                let scale = 1.0 / (1.0 - rate);
                for i in 0..out_grad.len() {
                    if out_data_cpu[i] == 0.0 {
                        p_grad[i] = 0.0;
                    } else {
                        p_grad[i] = out_grad[i] * scale;
                    }
                }
                parent.write().unwrap().add_cpu_grad(&p_grad);
            })),
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out_tensor: &TensorGraph<Self>| {
                ...
                parent
                    .write()
                    .unwrap()
                    .add_cpu_grad(out_tensor.get_cpu_grad());
            })),
        }))
    }

    fn layer_norm(
        node: &TensorNode<Self>,
        gamma_node: &TensorNode<Self>,
        beta_node: &TensorNode<Self>,
    ) -> TensorNode<Self> {
        let a = node.read().unwrap();
        let gamma = gamma_node.read().unwrap();
        let beta = beta_node.read().unwrap();
        let (a_cpu, _) = a.to_cpu().into_raw_vec_and_offset();
        let (g_cpu, _) = gamma.to_cpu().into_raw_vec_and_offset();
        let (b_cpu, _) = beta.to_cpu().into_raw_vec_and_offset();
        let cols = a.shape[1];
        let mut result = vec![0.0; a_cpu.len()];
        ...
        result
            ...
            .for_each(|(i, row_out)| {
                ...
                for j in 0..cols {
                    row_out[j] = ((row_in[j] - mean) / std_dev) * g_cpu[j] + b_cpu[j];
                }
            });
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            creators: vec![
                Arc::clone(node),
                Arc::clone(gamma_node),
                Arc::clone(beta_node),
            ],
            device: a.device.clone(),
            backward: Some(Box::new(|out_tensor: &TensorGraph<Self>| {
                let parent = &out_tensor.creators[0];
                let gamma_node = &out_tensor.creators[1];
                let beta_node = &out_tensor.creators[2];
                let out_grad = out_tensor.get_cpu_grad();
                let gamma_read = gamma_node.read().unwrap();
                let parent_read = parent.read().unwrap();
                let (g_data, _) = gamma_read.to_cpu().into_raw_vec_and_offset();
                let (p_data, _) = parent_read.to_cpu().into_raw_vec_and_offset();

                let cols = gamma_read.shape[0];
                let mut p_grad = vec![0.0; out_grad.len()];
                let mut g_grad = vec![0.0; cols];
    ...
    fn conv2d(image_node: &TensorNode<Self>, kernel_node: &TensorNode<Self>) -> TensorNode<Self> {
        let image = image_node.read().unwrap();
        let kernel = kernel_node.read().unwrap();
        let i_rows = image.shape[0];
        let i_cols = image.shape[1];
        let k_rows = kernel.shape[0];
        let k_cols = kernel.shape[1];
        let out_rows = i_rows - k_rows + 1;
        let out_cols = i_cols - k_cols + 1;
        ...
        let out_data = match (&image.data, &kernel.data) {
            ...
            _ => {
                let (i_data, _) = image.to_cpu().into_raw_vec_and_offset();
                let (k_data, _) = kernel.to_cpu().into_raw_vec_and_offset();
                ...
                for r in 0..out_rows {
                    for c in 0..out_cols {
                        ...
                        for kr in 0..k_rows {
                            for kc in 0..k_cols {
                                sum +=
                                    i_data[(r + kr) * i_cols + (c + kc)] * k_data[kr * k_cols + kc];
                            }
                        }
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            grad: None,
            creators: vec![Arc::clone(image_node), Arc::clone(kernel_node)],
            device: image.device.clone(),
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
                let image_node = &out_tensor.creators[0];
                let kernel_node = &out_tensor.creators[1];
                let out_grad = out_tensor.get_cpu_grad();
                let image = image_node.read().unwrap();
                let kernel = kernel_node.read().unwrap();
                let (i_data, _) = image.to_cpu().into_raw_vec_and_offset();
                let (k_data, _) = kernel.to_cpu().into_raw_vec_and_offset();
                let mut i_grad = vec![0.0; i_rows * i_cols];
                let mut k_grad = vec![0.0; k_rows * k_cols];
                ...
                for r in 0..out_rows {
                    for c in 0..out_cols {
                        ...
                        for kr in 0..k_rows {
                            for kc in 0..k_cols {
                                i_grad[(r + kr) * i_cols + (c + kc)] +=
                                    g * k_data[kr * k_cols + kc];
                                k_grad[kr * k_cols + kc] +=
                                    g * i_data[(r + kr) * i_cols + (c + kc)];
                            }
                        }
                    }
                }
    ...
    fn embedding(
        weights_node: &TensorNode<Self>,
        indices_matrix: &Array2<f32>,
    ) -> TensorNode<Self> {
            ...
            TensorData::Gpu(w_buf) => {
                if let Some((device, queue)) = weights.device.get_gpu() {
                    ...
                    let indices_buf = device.create_buffer(&wgpu::BufferDescriptor {
                        label: None,
                        size: (indices_f32.len() * 4) as wgpu::BufferAddress,
        ...
        Arc::new(RwLock::new(TensorGraph {
            data: out_data,
            shape: vec![total_tokens, hidden_size],
            grad: None,
            creators: vec![Arc::clone(weights_node)],
            ...
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
                ...
                for (seq_idx, &token_id) in indices.iter().enumerate() {
                    ...
                    for i in 0..hidden_size {
                        w_grad_calc[row_start + i] += out_grad[grad_start + i];
                    }
                }
    ...
    fn cross_entropy(logits_node: &TensorNode<Self>, targets: &Array2<f32>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            shape: vec![1, 1],
            grad: None,
            creators: vec![Arc::clone(logits_node)],
            device,
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
    ...
    fn mse(pred_node: &TensorNode<Self>, target: &Array2<f32>) -> TensorNode<Self> {
        ...
        for i in 0..p_data.len() {
            sum += (p_data[i] - target_vec[i]).powi(2);
        }
        let out_val = sum / p_data.len() as f32;

        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Cpu(vec![out_val]),
            shape: vec![1, 1],
            grad: None,
            creators: vec![Arc::clone(pred_node)],
            device: pred.device.clone(),
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
                ...
                for i in 0..p_data_cpu.len() {
                    grad_calc[i] = 2.0 * (p_data_cpu[i] - target_vec[i]) * out_grad / n;
                }
                drop(pred_read);
    ...
    fn huber_loss(node: &TensorNode<Self>, target: &Array2<f32>, delta: f32) -> TensorNode<Self> {
        ...
        for i in 0..p_data.len() {
            ...
            if error <= delta {
                sum += 0.5 * error.powi(2);
            } else {
                sum += delta * error - 0.5 * delta.powi(2);
            }
        }
        let out_val = sum / p_data.len() as f32;
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            device: pred.device.clone(),
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
                let p_node = &out_tensor.creators[0];
                let out_grad = out_tensor.get_cpu_grad()[0];
                ...
                for i in 0..p_data_cpu.len() {
                    ...
                    if diff.abs() <= delta {
                        grad_calc[i] = diff * out_grad / n;
                    } else {
                        grad_calc[i] = delta * diff.signum() * out_grad / n;
                    }
                }
                drop(p_read);
                p_node.write().unwrap().add_cpu_grad(&grad_calc);
            })),
        }))
    }
    ...
    fn cond(
        ...
        _false_val: &TensorNode<Self>,
    ) -> TensorNode<Self> {
        panic!(
            "ILLEGAL OP: `Cond` is a JIT Graph operator. In eager mode, use standard Rust `if/else` statements!"
        );
    }

    fn while_loop(_state: &TensorNode<Self>, _max_iters: usize) -> TensorNode<Self> {
        panic!(
            "ILLEGAL OP: `WhileLoop` is a JIT Graph operator. In eager mode, use standard Rust `for/while` loops!"
        );
    }
        ...
        let out_data = match &a.data {
            TensorData::Cpu(a_data) => {
                ...
                result
                    .par_iter_mut()
                    .zip(a_data.par_iter())
                    .for_each(|(res, &x)| *res = 1.0 / (1.0 + (-x).exp()));
                TensorData::Cpu(result)
            }
            TensorData::Gpu(a_buf) => {
                if let Some((device, queue)) = a.device.get_gpu() {
    ...
    fn max_pool2d(node: &TensorNode<Self>, kernel: usize) -> TensorNode<Self> {
        ...
        assert!(
            a.shape.len() == 4,
            "MaxPool2d expects 4D input [N, C, H, W]"
        );
                            ...
                            for ow in 0..out_w {
                                ...
                                for kh in 0..kernel {
                                    for kw in 0..kernel {
                                        ...
                                        let iw = ow * kernel + kw;
                                        let idx =
                                            n * (channels * h * w) + c * (h * w) + ih * w + iw;
                                        if a_data[idx] > max_val {
                                            max_val = a_data[idx];
                                        }
                                    }
                                }
                                result[n * (channels * out_h * out_w)
                                    + c * (out_h * out_w)
                                    + oh * out_w
                                    + ow] = max_val;
                            }
            ...
            TensorData::Gpu(a_buf) => {
                if let Some((device, queue)) = a.device.get_gpu() {
                    ...
                    let out_buf = device.create_buffer(&wgpu::BufferDescriptor {
                        label: None,
                        size: (total_out * 4) as wgpu::BufferAddress,
                        usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
                        mapped_at_creation: false,
                    });
                            ...
                            for ow in 0..out_w {
                                ...
                                for kh in 0..kernel {
                                    for kw in 0..kernel {
                                        ...
                                        let idx =
                                            n * (channels * h * w) + c * (h * w) + ih * w + iw;
                                        if p_data[idx] > max_val {
                                            max_val = p_data[idx];
                                            max_idx = idx;
                                        }
                                    }
                                }
                                p_grad[max_idx] += out_grad[n * (channels * out_h * out_w)
                                    + c * (out_h * out_w)
                                    + oh * out_w
                                    + ow];
                            }
    ...
    fn avg_pool2d(node: &TensorNode<Self>, kernel: usize) -> TensorNode<Self> {
        ...
        assert!(
            a.shape.len() == 4,
            "AvgPool2d expects 4D input [N, C, H, W]"
        );
        let (batch, channels, h, w) = (a.shape[0], a.shape[1], a.shape[2], a.shape[3]);
        let out_h = h / kernel;
        let out_w = w / kernel;
            ...
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
                ...
                for n in 0..batch {
                    for c in 0..channels {
                        for oh in 0..out_h {
                            for ow in 0..out_w {
                                let g_val = out_grad[n * (channels * out_h * out_w)
                                    + c * (out_h * out_w)
                                    + oh * out_w
                                    + ow]
                                    / kernel_area;
    ...
    fn batch_norm(
        ...
        let b_read = b.read().unwrap();
        let rm_read = rm.read().unwrap();
        let rv_read = rv.read().unwrap();

        let n = if x_read.shape.len() == 4 {
            x_read.shape[0]
        } else {
            1
        };
        let c = if x_read.shape.len() == 4 {
            x_read.shape[1]
        } else {
            x_read.shape[0]
        };
        let h = if x_read.shape.len() == 4 {
            x_read.shape[2]
        } else {
            1
        };
        let w = if x_read.shape.len() == 4 {
            x_read.shape[3]
        } else {
            x_read.shape[1]
        };
        ...
        let out_data = match (
            ...
            (
                TensorData::Cpu(x_data),
                TensorData::Cpu(g_data),
                TensorData::Cpu(b_data),
                TensorData::Cpu(rm_data),
                TensorData::Cpu(rv_data),
            ) => {
            ...
            (
                TensorData::Gpu(x_buf),
                TensorData::Gpu(g_buf),
                TensorData::Gpu(b_buf),
                TensorData::Gpu(rm_buf),
                TensorData::Gpu(rv_buf),
            ) => {
                if let Some((device, queue)) = x_read.device.get_gpu() {
                    ...
                    let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                        ...
                        entries: &[
                            ...
                            wgpu::BindGroupEntry {
                                binding: 4,
                                resource: rm_buf.as_entire_binding(),
                            },
                            wgpu::BindGroupEntry {
                                binding: 5,
                                resource: rv_buf.as_entire_binding(),
                            },
                            wgpu::BindGroupEntry {
                                binding: 6,
                                resource: c_buf.as_entire_binding(),
        ...
        Arc::new(RwLock::new(TensorGraph {
            data: out_data,
            shape: x_read.shape.clone(),
            grad: None,
            creators: vec![Arc::clone(x), Arc::clone(g), Arc::clone(b)],
            device: x_read.device.clone(),
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
                let x_node = &out_tensor.creators[0];
                let g_node = &out_tensor.creators[1];
                let b_node = &out_tensor.creators[2];
                let out_grad = out_tensor.get_cpu_grad();

                let x_read = x_node.read().unwrap();
                let g_read = g_node.read().unwrap();
                let (x_data, _) = x_read.to_cpu().into_raw_vec_and_offset();
                let (g_data, _) = g_read.to_cpu().into_raw_vec_and_offset();

                let mut x_grad = vec![0.0; x_data.len()];
                let mut gamma_grad = vec![0.0; c];
                let mut beta_grad = vec![0.0; c];
    ...
    fn conv1d(i_node: &TensorNode<Self>, k_node: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            data: out_data,
            shape: vec![batch, out_c, out_len],
            grad: None,
            creators: vec![Arc::clone(i_node), Arc::clone(k_node)],
            device: i.device.clone(),
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
                let i_node = &out_tensor.creators[0];
                let k_node = &out_tensor.creators[1];
                let out_grad = out_tensor.get_cpu_grad();
                let i_read = i_node.read().unwrap();
                let k_read = k_node.read().unwrap();
                let (i_data, _) = i_read.to_cpu().into_raw_vec_and_offset();
                let (k_data, _) = k_read.to_cpu().into_raw_vec_and_offset();

                let mut i_grad = vec![0.0; batch * in_c * len];
                let mut k_grad = vec![0.0; out_c * in_c * k_len];
                ...
                for b in 0..batch {
                    for oc in 0..out_c {
                        for ol in 0..out_len {
                            ...
                            for ic in 0..in_c {
                                for kl in 0..k_len {
                                    i_grad[b * (in_c * len) + ic * len + (ol + kl)] +=
                                        g * k_data[oc * (in_c * k_len) + ic * k_len + kl];
                                    k_grad[oc * (in_c * k_len) + ic * k_len + kl] +=
                                        g * i_data[b * (in_c * len) + ic * len + (ol + kl)];
                                }
                            }
                        }
                    }
                ...
                drop(i_read);
                drop(k_read);
                i_node.write().unwrap().add_cpu_grad(&i_grad);
                k_node.write().unwrap().add_cpu_grad(&k_grad);
            })),
        }))
    }

    fn topk(_a: &TensorNode<Self>, _k: usize) -> (TensorNode<Self>, TensorNode<Self>) {
        panic!("TopK WGSL Shader pending! Adding this allows the LazyTracer to map the IR.");
    }
    ...
    fn conv3d(i_node: &TensorNode<Self>, k_node: &TensorNode<Self>) -> TensorNode<Self> {
                            ...
                            for od in 0..out_d {
                                for oh in 0..out_h {
                                    for ow in 0..out_w {
                                        ...
                                        let out_idx = oc * (out_d * out_h * out_w)
                                            + od * (out_h * out_w)
                                            + oh * out_w
                                            + ow;
        ...
        Arc::new(RwLock::new(TensorGraph {
            data: out_data,
            shape: vec![batch, out_c, out_d, out_h, out_w],
            grad: None,
            creators: vec![Arc::clone(i_node), Arc::clone(k_node)],
            device: i.device.clone(),
            backward: Some(Box::new(move |out_tensor: &TensorGraph<Self>| {
                ...
                let mut i_grad = vec![0.0; batch * in_c * in_d * in_h * in_w];
                let mut k_grad = vec![0.0; out_c * in_c * k_d * k_h * k_w];
                ...
                for b in 0..batch {
                    for oc in 0..out_c {
                        for od in 0..out_d {
                            for oh in 0..out_h {
                                for ow in 0..out_w {
                                    let g = out_grad[b * (out_c * out_d * out_h * out_w)
...
pub static GLOBAL_COMPUTE_GRAPH: std::sync::Mutex<ComputeGraph> =
    std::sync::Mutex::new(ComputeGraph { nodes: Vec::new() });
