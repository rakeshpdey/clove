...
//! The LazyBackend module compiles eager operations into a fused JIT graph.
//! It intercepts tensor math, builds a Directed Acyclic Graph (DAG) of operations,
//! performs optimization passes (DCE, Constant Folding, Fusion), and generates
//! highly-optimized WGSL compute shaders for WebGPU execution.

use crate::backend::{Backend, ComputeGraph, Opcode, Precision, SymInt, TensorData};
...
// THE GLOBAL TRACER
// NOTE: We use thread-local storage to seamlessly intercept tensor operations
// without requiring a context object to be passed around everywhere.
thread_local! {
    /// Thread-local global computation graph to trace operations seamlessly.
    pub static GLOBAL_GRAPH: std::cell::RefCell<ComputeGraph> = std::cell::RefCell::new(ComputeGraph::new());
}

/// The lazy execution backend that intercepts tensor operations instead of executing them immediately.
pub struct LazyBackend;

impl LazyBackend {
    /// Extracts the internal IR graph ID from a lazy tensor node.
    pub fn get_id(node: &TensorNode<Self>) -> usize {
        if let TensorData::Lazy(id) = node.read().unwrap().data {
            id
        } else {
            panic!("Not a lazy tensor!");
        }
    }

    /// Accumulates gradients dynamically into the computational graph.
    pub fn accumulate_grad(node: &TensorNode<Self>, new_grad_id: usize) {
        ...
        if let Some(TensorData::Lazy(existing_id)) = n.grad {
            ...
            let sum_id = GLOBAL_GRAPH.with(|g| {
                g.borrow_mut()
                    .push(Opcode::Add, shape, vec![existing_id, new_grad_id])
            });
    ...
    /// Helper to convert standard usize dimensions into Symbolic Integers.
    fn to_sym(shape: &[usize]) -> Vec<SymInt> {
        shape.iter().map(|&x| SymInt::Const(x)).collect()
    }

    /// Explicitly marks a dimension as symbolic (dynamic) for tracing.
...
// NOTE: Operations here push IRNodes to the GLOBAL_GRAPH rather than computing data.
impl Backend for LazyBackend {
    ...
    fn new(_data_array: Array2<f32>) -> TensorNode<Self> {
        panic!("Use new_cpu for Lazy nodes")
    }
    ...
    fn clone_tensor(
        primitive: &Self::TensorPrimitive,
        _device: &Self::Device,
    ...
    // NOTE: Hard stops for eager methods that don't apply to the Lazy tracer.
    fn get_cpu_grad(_: &TensorGraph<Self>) -> &[f32] {
        panic!("Cannot get grad from Lazy node")
    }
    fn add_cpu_grad(_: &mut TensorGraph<Self>, _: &[f32]) {
        panic!("Cannot add raw grad array to Lazy node")
    }
    fn clip_gradients(_: &mut TensorGraph<Self>) {}
    fn to_cpu(_: &TensorGraph<Self>) -> Array2<f32> {
        panic!("Compile graph first!")
    }
    fn to_gpu(_: &mut TensorGraph<Self>, _: Arc<wgpu::Device>, _: Arc<wgpu::Queue>) {
        panic!("Compile graph first!")
    }
    fn grad_to_cpu(_: &TensorGraph<Self>) -> Option<Array2<f32>> {
        panic!("Compile graph first!")
    }
    ...
    fn cast(a: &TensorNode<Self>, precision: Precision) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Cast(precision), sym_shape, vec![a_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: a.read().unwrap().shape.clone(),
            grad: None,
            creators: vec![a.clone()],
            device: a.read().unwrap().device.clone(),
            backward: Some(Box::new(|out| {
                let out_grad_id = if let TensorData::Lazy(id) = out.grad.as_ref().unwrap() {
                    *id
                } else {
                    unreachable!()
                };
    ...
    fn all_reduce(a: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::AllReduce, sym_shape, vec![a_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: a.read().unwrap().shape.clone(),
            ...
            backward: Some(Box::new(|out| {
                ...
                let da_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut()
                        .push(Opcode::AllReduce, sym, vec![out_grad_id])
                });
    ...
    fn add(a: &TensorNode<Self>, b: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Add, sym_shape, vec![a_id, b_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: a.read().unwrap().shape.clone(),
            grad: None,
            creators: vec![a.clone(), b.clone()],
            device: a.read().unwrap().device.clone(),
            backward: Some(Box::new(|out| {
                let out_grad_id = if let TensorData::Lazy(id) = out.grad.as_ref().unwrap() {
                    *id
    ...
    fn mul(a: &TensorNode<Self>, b: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Mul, sym_shape, vec![a_id, b_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: a.read().unwrap().shape.clone(),
            ...
            backward: Some(Box::new(|out| {
                ...
                let a_id = Self::get_id(&out.creators[0]);
                let b_id = Self::get_id(&out.creators[1]);
                let sym = GLOBAL_GRAPH.with(|g| g.borrow().nodes[out_grad_id].shape.clone());
                let da_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut()
                        .push(Opcode::Mul, sym.clone(), vec![out_grad_id, b_id])
                });
                let db_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut()
                        .push(Opcode::Mul, sym, vec![out_grad_id, a_id])
                });
                Self::accumulate_grad(&out.creators[0], da_id);
    ...
    fn matmul(a: &TensorNode<Self>, b: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::MatMul, sym_shape.clone(), vec![a_id, b_id])
        });

        let mut eager_shape = a.read().unwrap().shape.clone();
        let eager_last = eager_shape.len() - 1;
        eager_shape[eager_last] = b.read().unwrap().shape[1];
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out| {
                ...
                let b_t_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut().push(
                        Opcode::Transpose,
                        vec![b_sym[1].clone(), b_sym[0].clone()],
                        vec![b_id],
                    )
                });
                let da_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut()
                        .push(Opcode::MatMul, a_sym.clone(), vec![out_grad_id, b_t_id])
                });

                let a_t_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut().push(
                        Opcode::Transpose,
                        vec![a_sym[1].clone(), a_sym[0].clone()],
                        vec![a_id],
                    )
                });
                let db_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut()
                        .push(Opcode::MatMul, b_sym, vec![a_t_id, out_grad_id])
                });
    ...
    fn relu(a: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out| {
                ...
                let da_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut()
                        .push(Opcode::ReluGrad, sym, vec![a_id, out_grad_id])
                });
                Self::accumulate_grad(&out.creators[0], da_id);
            })),
        }))
    ...
    fn gelu(a: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out| {
                ...
                let da_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut()
                        .push(Opcode::GeluGrad, sym, vec![a_id, out_grad_id])
                });
                Self::accumulate_grad(&out.creators[0], da_id);
            })),
        }))
    ...
    fn softmax(a: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out| {
                ...
                let out_data_id = if let TensorData::Lazy(id) = out.data {
                    id
                } else {
                    unreachable!()
                };
                ...
                let da_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut()
                        .push(Opcode::SoftmaxGrad, sym, vec![out_data_id, out_grad_id])
                });
                Self::accumulate_grad(&out.creators[0], da_id);
            })),
        }))
    ...
    fn layer_norm(
        ...
        let a_id = Self::get_id(a);
        let g_id = Self::get_id(g);
        let b_id = Self::get_id(b);
        let sym_shape = GLOBAL_GRAPH.with(|graph| graph.borrow().nodes[a_id].shape.clone());
        let id = GLOBAL_GRAPH.with(|graph| {
            graph
                .borrow_mut()
                .push(Opcode::LayerNorm, sym_shape, vec![a_id, g_id, b_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: a.read().unwrap().shape.clone(),
            grad: None,
            creators: vec![a.clone(), g.clone(), b.clone()],
            device: a.read().unwrap().device.clone(),
            backward: None,
        }))
    }
    ...
    fn sub(a: &TensorNode<Self>, b: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Sub, sym_shape, vec![a_id, b_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: a.read().unwrap().shape.clone(),
    ...
    fn mul_scalar(a: &TensorNode<Self>, scalar: f32) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::ScalarMul(scalar), sym_shape, vec![a_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: a.read().unwrap().shape.clone(),
    ...
    fn rope(a: &TensorNode<Self>, pos_offset: usize, head_dim: usize) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::RoPE(pos_offset, head_dim), sym_shape, vec![a_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: a.read().unwrap().shape.clone(),
    ...
    fn concat_seq(a: &TensorNode<Self>, b: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        sym_shape[seq_dim] = SymInt::Add(
            Box::new(a_sym[seq_dim].clone()),
            Box::new(b_sym[seq_dim].clone()),
        );

        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Concat, sym_shape, vec![a_id, b_id])
        });
    ...
    fn transpose(a: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            backward: Some(Box::new(|out| {
                ...
                let da_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut()
                        .push(Opcode::Transpose, a_sym, vec![out_grad_id])
                });
                Self::accumulate_grad(&out.creators[0], da_id);
            })),
        }))
    ...
    fn dropout(a: &TensorNode<Self>, rate: f32) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Dropout(rate), sym_shape, vec![a_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: a.read().unwrap().shape.clone(),
    ...
    fn conv2d(i: &TensorNode<Self>, k: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let out_rows = SymInt::Add(
            Box::new(SymInt::Sub(
                Box::new(i_sym[0].clone()),
                Box::new(k_sym[0].clone()),
            )),
            Box::new(SymInt::Const(1)),
        );
        let out_cols = SymInt::Add(
            Box::new(SymInt::Sub(
                Box::new(i_sym[1].clone()),
                Box::new(k_sym[1].clone()),
            )),
            Box::new(SymInt::Const(1)),
        );
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Conv2d, vec![out_rows, out_cols], vec![i_id, k_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: vec![1, 1],
            grad: None,
            creators: vec![i.clone(), k.clone()],
            device: i.read().unwrap().device.clone(),
            backward: None,
        }))
    }
    ...
    fn embedding(w: &TensorNode<Self>, indices: &Array2<f32>) -> TensorNode<Self> {
        ...
        let idx_id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Input, vec![seq_len.clone()], vec![])
        });
        ...
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: vec![indices.len(), w.read().unwrap().shape[1]],
            grad: None,
            creators: vec![w.clone()],
            device: w.read().unwrap().device.clone(),
            backward: None,
        }))
    }

    fn cross_entropy(l: &TensorNode<Self>, targets: &Array2<f32>) -> TensorNode<Self> {
        // dynamically push the CPU targets into the JIT graph so they can be accessed natively during backward passes!
        let targets_vec: Vec<f32> = targets.iter().cloned().collect();
        let t_id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut().push(
                Opcode::Input,
                vec![SymInt::Const(targets_vec.len())],
                vec![],
            )
        });
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            shape: vec![1],
            grad: None,
            creators: vec![l.clone()],
            device: l.read().unwrap().device.clone(),
            backward: Some(Box::new(move |out| {
                let out_grad_id = if let TensorData::Lazy(id) = out.grad.as_ref().unwrap() {
                    *id
                } else {
                ...
                let dl_id = GLOBAL_GRAPH.with(|g| {
                    g.borrow_mut().push(
                        Opcode::CrossEntropyGrad,
                        sym,
                        vec![l_id, t_id, out_grad_id],
                    )
                });
    ...
    fn mse(p: &TensorNode<Self>, _targets: &Array2<f32>) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::MSE, vec![SymInt::Const(1)], vec![p_id, t_id])
        });
        ...
        Arc::new(RwLock::new(TensorGraph {
            ...
            shape: vec![1],
            grad: None,
            creators: vec![p.clone()],
            device: p.read().unwrap().device.clone(),
            backward: None,
        }))
    }

    // LAZY TRACER IMPLEMENTATIONS
    ...
    fn cond(
        condition: &TensorNode<Self>,
        true_val: &TensorNode<Self>,
        false_val: &TensorNode<Self>,
    ) -> TensorNode<Self> {
        let c_id = Self::get_id(condition);
        let t_id = Self::get_id(true_val);
        let f_id = Self::get_id(false_val);
        let sym_shape = GLOBAL_GRAPH.with(|g| g.borrow().nodes[t_id].shape.clone());
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Cond, sym_shape, vec![c_id, t_id, f_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: true_val.read().unwrap().shape.clone(),
            grad: None,
            creators: vec![condition.clone(), true_val.clone(), false_val.clone()],
            device: true_val.read().unwrap().device.clone(),
            backward: None,
        }))
    }
    ...
    fn while_loop(state: &TensorNode<Self>, max_iters: usize) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::WhileLoop(max_iters), sym_shape, vec![s_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: state.read().unwrap().shape.clone(),
            grad: None,
            creators: vec![state.clone()],
            device: state.read().unwrap().device.clone(),
            backward: None,
        }))
    }
    ...
    fn paged_attention(
        ...
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: q.read().unwrap().shape.clone(),
            grad: None,
            creators: vec![
                q.clone(),
                k.clone(),
            ...
            device: q.read().unwrap().device.clone(),
            backward: None,
        }))
    }
    ...
    fn max_pool2d(a: &TensorNode<Self>, k: usize) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::MaxPool2d(k), out_sym, vec![a_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: out_shape,
    ...
    fn avg_pool2d(a: &TensorNode<Self>, k: usize) -> TensorNode<Self> {
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::AvgPool2d(k), out_sym, vec![a_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: out_shape,
    ...
    fn batch_norm(
        ...
        let x_id = Self::get_id(x);
        let g_id = Self::get_id(g);
        let b_id = Self::get_id(b);
        let rm_id = Self::get_id(rm);
        let rv_id = Self::get_id(rv);
        let sym = GLOBAL_GRAPH.with(|gr| gr.borrow().nodes[x_id].shape.clone());
        let id = GLOBAL_GRAPH.with(|gr| {
            gr.borrow_mut()
                .push(Opcode::BatchNorm, sym, vec![x_id, g_id, b_id, rm_id, rv_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: x.read().unwrap().shape.clone(),
            grad: None,
            creators: vec![x.clone(), g.clone(), b.clone()],
            device: x.read().unwrap().device.clone(),
            backward: None,
        }))
    }

    // LAZY TRACER IMPLEMENTATIONS (CONV1D & CONV3D)
    fn conv1d(i: &TensorNode<Self>, k: &TensorNode<Self>) -> TensorNode<Self> {
        let i_id = Self::get_id(i);
        let k_id = Self::get_id(k);
        let i_read = i.read().unwrap();
        let k_read = k.read().unwrap();
        let i_sym = GLOBAL_GRAPH.with(|g| g.borrow().nodes[i_id].shape.clone());
        let k_sym = GLOBAL_GRAPH.with(|g| g.borrow().nodes[k_id].shape.clone());

        let out_l = SymInt::Add(
            Box::new(SymInt::Sub(
                Box::new(i_sym[2].clone()),
                Box::new(k_sym[2].clone()),
            )),
            Box::new(SymInt::Const(1)),
        );
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Conv1d, out_sym, vec![i_id, k_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: out_shape,
    ...
    fn conv3d(i: &TensorNode<Self>, k: &TensorNode<Self>) -> TensorNode<Self> {
        ...
        let out_h = SymInt::Add(
            Box::new(SymInt::Sub(
                Box::new(i_sym[3].clone()),
                Box::new(k_sym[3].clone()),
            )),
            Box::new(SymInt::Const(1)),
        );
        let out_w = SymInt::Add(
            Box::new(SymInt::Sub(
                Box::new(i_sym[4].clone()),
                Box::new(k_sym[4].clone()),
            )),
            Box::new(SymInt::Const(1)),
        );
        ...
        let id = GLOBAL_GRAPH.with(|g| {
            g.borrow_mut()
                .push(Opcode::Conv3d, out_sym, vec![i_id, k_id])
        });
        Arc::new(RwLock::new(TensorGraph {
            data: TensorData::Lazy(id),
            shape: out_shape,
    ...
    // NOTE: TopK is a complex operator returning two outputs (Values and Indices).
    // The JIT handles this by pushing two identical Opcode nodes to the graph,
    // generating two separate TensorData::Lazy paths.
    fn topk(a: &TensorNode<Self>, k: usize) -> (TensorNode<Self>, TensorNode<Self>) {
        let a_read = a.read().unwrap();
        let a_id = match a_read.data {
            crate::backend::TensorData::Lazy(id) => id,
            _ => unreachable!("LazyBackend requires Lazy tensor data"),
        };

        let mut out_shape = a_read.shape.clone();
        if let Some(last) = out_shape.last_mut() {
            *last = k; // TopK reduces the last dimension to size K
        }

        let mut graph = crate::backend::GLOBAL_COMPUTE_GRAPH.lock().unwrap();
        let sym_shape: Vec<_> = out_shape
            .iter()
            .map(|&s| crate::backend::SymInt::Const(s))
            .collect();

        // Push the operation into the Lazy Intermediate Representation
        let val_id = graph.push(
            crate::backend::Opcode::TopK(k),
            sym_shape.clone(),
            vec![a_id],
        );
        let idx_id = graph.push(crate::backend::Opcode::TopK(k), sym_shape, vec![a_id]);

        let val_node = std::sync::Arc::new(std::sync::RwLock::new(crate::tensor::TensorGraph {
            data: crate::backend::TensorData::Lazy(val_id),
            shape: out_shape.clone(),
            grad: None,
            backward: None,
            creators: vec![std::sync::Arc::clone(a)],
            device: a_read.device.clone(),
        }));

        let idx_node = std::sync::Arc::new(std::sync::RwLock::new(crate::tensor::TensorGraph {
            data: crate::backend::TensorData::Lazy(idx_id),
            shape: out_shape,
            grad: None,
            backward: None,
            creators: vec![std::sync::Arc::clone(a)],
...
/// Represents an executable step in the compiled GPU schedule.
/// Each enum variant maps to a distinct WGSL shader compute dispatch.
pub enum Step {
    Fused {
        code: String,
        input_ids: Vec<usize>,
    ...
    HorizontalFusionGroup {
        matmuls: Vec<(usize, usize, usize, SymInt, SymInt, SymInt)>,
        uniform_bufs: Vec<Arc<wgpu::Buffer>>,
    },
    Transpose {
        in_id: usize,
        out_id: usize,
...
impl Step {
    pub fn get_inputs(&self) -> Vec<usize> {
        match self {
            ...
            Step::SoftmaxGrad {
                ...
                out_grad_id,
                ..
            } => vec![*out_data_id, *out_grad_id],
            Step::LayerNorm {
                in_id,
                gamma_id,
                beta_id,
                ..
            } => vec![*in_id, *gamma_id, *beta_id],
            Step::FlashAttention {
                q_id, k_id, v_id, ..
            } => vec![*q_id, *k_id, *v_id],
            Step::HorizontalFusionGroup { matmuls, .. } => matmuls
                .iter()
                .flat_map(|&(a, b, _, _, _, _)| vec![a, b])
                .collect(),
            ...
            Step::BatchNorm {
                ...
                rv_id,
                ..
            } => vec![*x_id, *g_id, *b_id, *rm_id, *rv_id],
    ...
    pub fn get_outputs(&self) -> Vec<(usize, SymInt)> {
        match self {
            Step::Fused {
                out_id, out_size, ..
            } => vec![(*out_id, out_size.clone())],
            Step::MatMul { out_id, m, n, .. } => vec![(
                *out_id,
                SymInt::Mul(Box::new(m.clone()), Box::new(n.clone())),
            )],
            Step::Softmax {
                out_id, rows, cols, ..
            } => vec![(
                *out_id,
                SymInt::Mul(Box::new(rows.clone()), Box::new(cols.clone())),
            )],
            Step::SoftmaxGrad {
                out_id, rows, cols, ..
            } => vec![(
            ...
            Step::FlashAttention {
                ...
                *out_id,
                SymInt::Mul(
                    Box::new(seq_len.clone()),
                    Box::new(SymInt::Const(*head_dim as usize)),
                ),
            )],
            Step::HorizontalFusionGroup { matmuls, .. } => matmuls
                .iter()
                .map(|&(_, _, out, ref m, _, ref n)| {
                    (out, SymInt::Mul(Box::new(m.clone()), Box::new(n.clone())))
                })
                .collect(),
            Step::Transpose {
                out_id, rows, cols, ..
            ...
            Step::Embedding {
                ...
                *out_id,
                SymInt::Mul(Box::new(seq_len.clone()), Box::new(hidden_size.clone())),
            )],
...
// NOTE: These optimization passes run linearly through the topological graph,
// cutting out redundant nodes before physical execution begins.
fn perform_dce(graph: &mut ComputeGraph, required_outputs: &[usize]) {
    ...
    println!(
        "DEAD CODE ELIMINATION: Removed {} useless operations.",
        original_size - graph.nodes.len()
    );
}
...
fn perform_constant_folding(graph: &mut ComputeGraph) {
    ...
    if rewired > 0 {
        println!(
            "CONSTANT FOLDING: Graph algebraically simplified. Bypassed {} redundant operations.",
            rewired
        );
    }
}
...
pub fn build_execution_plan(graph: &mut ComputeGraph, device: &wgpu::Device) -> Vec<Step> {
    ...
    for i in 0..graph.nodes.len() {
        if skip_nodes.contains(&graph.nodes[i].id) {
            continue;
        }
        let node = &graph.nodes[i];

        // Advanced matching for FlashAttention pattern (MatMul -> Softmax -> MatMul)
        if matches!(node.op, Opcode::MatMul) && node.dependencies.len() == 2 {
            let softmax_id = node.dependencies[0];
            let v_id = node.dependencies[1];

            if let Some(softmax_node) = graph
                .nodes
                .iter()
                .find(|n| n.id == softmax_id && matches!(n.op, Opcode::Softmax))
            {
                let inner_matmul_id = softmax_node.dependencies[0];
                if let Some(inner_matmul) = graph
                    .nodes
                    .iter()
                    .find(|n| n.id == inner_matmul_id && matches!(n.op, Opcode::MatMul))
                {
                    ...
                    steps.push(Step::FlashAttention {
                        q_id,
                        k_id,
                        v_id,
                        out_id: node.id,
    ...
    // NOTE: This closure handles horizontal and vertical fusion of element-wise operators.
    // It dynamically writes WGSL code on the fly to stitch operations together into a single mega-kernel.
    let flush_fused = |fused: &mut Vec<crate::backend::IRNode>, steps: &mut Vec<Step>| {
        ...
        for n in fused.iter() {
            for &dep in &n.dependencies {
                if !fused.iter().any(|fn_node| fn_node.id == dep) && !input_ids.contains(&dep) {
                    input_ids.push(dep);
                }
            }
        }
        ...
        for (i, &in_id) in input_ids.iter().enumerate() {
        ...
        wgsl.push_str(&format!(
            "@group(0) @binding({}) var<storage, read_write> outputs: array<f32>;\n\n",
            out_binding
        ));
        ...
        for n in fused.iter() {
            match n.op.clone() {
                ...
                Opcode::ReluGrad => {
                    let a = format_operand(n.dependencies[0], &input_ids, graph);
                    let g_out = format_operand(n.dependencies[1], &input_ids, graph);
                    wgsl.push_str(&format!(
                        "    let var_{} = select(0.0, {}, {} > 0.0);\n",
                        n.id, g_out, a
                    ));
                }
                Opcode::GELU => {
    ...
    for node in &graph.nodes {
        if skip_nodes.contains(&node.id) || matches!(node.op, Opcode::Input) {
            continue;
        }
        match node.op.clone() {
            Opcode::MatMul => {
            ...
            Opcode::SoftmaxGrad => {
                ...
                steps.push(Step::SoftmaxGrad {
                    out_data_id: node.dependencies[0],
                    out_grad_id: node.dependencies[1],
                    out_id: node.id,
                    rows,
                    cols,
                    uniform_buf,
            ...
            Opcode::LayerNorm => {
                ...
                steps.push(Step::LayerNorm {
                    in_id: in_node.id,
                    gamma_id: node.dependencies[1],
                    beta_id: node.dependencies[2],
                    out_id: node.id,
                    rows,
                    cols,
                    uniform_buf,
            ...
            Opcode::Embedding => {
                ...
                let w_node = graph
                    ...
                    .find(|n| n.id == node.dependencies[0])
                    .unwrap();
                let vocab_size = w_node.shape[0].clone();
                let hidden_size = w_node.shape[1].clone();
                ...
                steps.push(Step::Embedding {
                    w_id: node.dependencies[0],
                    idx_id: node.dependencies[1],
                    out_id: node.id,
                    vocab_size,
                    hidden_size,
                    seq_len,
            ...
            Opcode::CrossEntropy => {
                ...
                let l_node = graph
                    ...
                    .find(|n| n.id == node.dependencies[0])
                    .unwrap();
                let rows = l_node.shape[0].clone();
                let cols = l_node.shape[1].clone();
                let uniform_buf = Arc::new(device.create_buffer(&wgpu::BufferDescriptor {
                    label: None,
                    size: 32,
                    usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
                ...
                steps.push(Step::CrossEntropy {
                    l_id: node.dependencies[0],
                    t_id: node.dependencies[1],
                    out_id: node.id,
                    rows,
                    cols,
                    uniform_buf,
            ...
            Opcode::CrossEntropyGrad => {
                ...
                steps.push(Step::CrossEntropyGrad {
                    l_id: node.dependencies[0],
                    t_id: node.dependencies[1],
                    out_grad_id: node.dependencies[2],
                    out_id: node.id,
                    rows,
                    cols,
                    uniform_buf,
            ...
            Opcode::MaxPool2d(k) => {
                ...
                let (n, c, h, w) = (
                    ...
                    in_node.shape[1].clone(),
                    in_node.shape[2].clone(),
                    in_node.shape[3].clone(),
                );
            ...
            Opcode::BatchNorm => {
                ...
                steps.push(Step::BatchNorm {
                    x_id: node.dependencies[0],
                    g_id: node.dependencies[1],
                    b_id: node.dependencies[2],
                    rm_id: node.dependencies[3],
                    rv_id: node.dependencies[4],
                    out_id: node.id,
                    out_size,
                    n,
                    c,
            ...
            Opcode::Conv1d => {
                ...
                steps.push(Step::Conv1d {
                    i_id: node.dependencies[0],
                    k_id: node.dependencies[1],
                    out_id: node.id,
                    out_size,
                    n: in_node.shape[0].clone(),
                    c: in_node.shape[1].clone(),
                    l: in_node.shape[2].clone(),
                    out_c: k_node.shape[0].clone(),
                    k_l: k_node.shape[2].clone(),
                    uniform_buf,
                });
            }
            Opcode::Conv3d => {
                ...
                steps.push(Step::Conv3d {
                    ...
                    n: in_node.shape[0].clone(),
                    c: in_node.shape[1].clone(),
                    d: in_node.shape[2].clone(),
                    h: in_node.shape[3].clone(),
                    w: in_node.shape[4].clone(),
                    out_c: k_node.shape[0].clone(),
                    k_d: k_node.shape[2].clone(),
                    k_h: k_node.shape[3].clone(),
                    k_w: k_node.shape[4].clone(),
                    uniform_buf,
                });
            }
            Opcode::Cond => {
                ...
                steps.push(Step::Cond {
                    c_id: node.dependencies[0],
                    t_id: node.dependencies[1],
                    f_id: node.dependencies[2],
                    out_id: node.id,
                    size,
                    uniform_buf,
                });
            ...
            Opcode::WhileLoop(max_iters) => {
                ...
                steps.push(Step::WhileLoop {
                    s_id: node.dependencies[0],
                    out_id: node.id,
                    size,
                    max_iters,
                    uniform_buf,
            ...
            Opcode::PagedAttention => {
                ...
                // Dynamically extract head_dim from the Query tensor's shape (last dimension)
                let q_node = graph
                    .nodes
                    .iter()
                    .find(|n| n.id == node.dependencies[0])
                    .unwrap();
                let head_dim = match q_node.shape.last() {
                    Some(SymInt::Const(v)) => *v as u32,
                    _ => 64, // Fallback default
                };

                steps.push(Step::PagedAttention {
                    q_id: node.dependencies[0],
                    k_id: node.dependencies[1],
                    v_id: node.dependencies[2],
                    kv_id: node.dependencies[3],
                    bt_id: node.dependencies[4],
                    cl_id: node.dependencies[5],
                    out_id: node.id,
                    size,
                    head_dim,
                    block_size: 16,
                    uniform_buf, // Assuming standard 16-token vLLM block
                });
            }
            Opcode::TopK(k) => {
                flush_fused(&mut fused_nodes, &mut steps);
                let in_node = graph
                    ...
                    .find(|n| n.id == node.dependencies[0])
                    .unwrap();
                let last = in_node.shape.len() - 1;
                let cols = in_node.shape[last].clone();
                let rows = if last == 0 {
                    SymInt::Const(1)
                } else {
                    SymInt::multiply_all(&in_node.shape[0..last])
                };

                if !steps.iter().any(|s| {
                    if let Step::TopK { in_id, .. } = s {
                        *in_id == in_node.id
                    } else {
                        false
                    }
                    ...
                    let outs: Vec<_> = graph
                        ...
                        .iter()
                        .filter(|n| {
                            matches!(n.op, Opcode::TopK(_)) && n.dependencies[0] == in_node.id
                        })
                        .collect();
                    if outs.len() == 2 {
                        let uniform_buf = Arc::new(device.create_buffer(&wgpu::BufferDescriptor {
                        ...
                        steps.push(Step::TopK {
                            in_id: in_node.id,
                            val_out_id: outs[0].id,
                            idx_out_id: outs[1].id,
                            k,
                            rows,
                            cols,
                            uniform_buf,
    ...
    while i < steps.len() {
        ...
        } = steps[i]
        {
            ...
            if group.len() > 1 {
                println!(
                    "HORIZONTAL FUSION: Grouped {} MatMuls into a Parallel Dispatch!",
                    group.len()
                );
                optimized_steps.push(Step::HorizontalFusionGroup {
                    matmuls: group,
                    uniform_bufs: unifs,
                });
                i = j;
...
// NOTE: Liveness analysis tracks when a virtual memory buffer is accessed for the
// last time during execution. Once released, its memory space is recycled for future nodes.
fn perform_liveness_analysis(steps: &[Step]) -> HashMap<usize, usize> {
    ...
    for (i, step) in steps.iter().enumerate() {
        ...
        for in_id in step.get_inputs() {
            if let Some(&last_step) = last_used.get(&in_id)
                && last_step == i
                && let Some(&vid) = buffer_map.get(&in_id)
            {
                free_pools.push(vid);
            }
        }
    ...
    println!(
        "VIRTUAL POOLING: Assigned {} dynamic memory pools.",
        next_vid
    );
    buffer_map
}
...
/// A fully compiled and optimized neural network graph ready for high-speed GPU execution.
pub struct CompiledModel {
    ...
    // Hardware Caches to completely eliminate Driver Overhead
    pub bind_groups: RwLock<HashMap<usize, Vec<Arc<wgpu::BindGroup>>>>,
}
    ...
    pub fn execute_with_sync(
        ...
        for step in &self.steps {
            for (out_id, sym_shape) in step.get_outputs() {
                ...
                let resize_needed = if let Some(buf) = buffers_mut.get(&virtual_id) {
                    buf.size() < required_bytes
                } else {
                    true
                };

                if resize_needed {
                    let allocated_bytes = if required_bytes < 1024 {
                        1024
                    } else {
                        required_bytes * 2
                    };
                    let new_buf = device.create_buffer(&wgpu::BufferDescriptor {
                        label: Some("Dynamic Resized Buffer"),
        ...
        // If memory pools were rebuilt, it MUST invalidate the cached driver memory pointers!
        if resized_any {
            self.bind_groups.write().unwrap().clear();
        }
        ...
        // Pull the cache lock once per execution
        let mut bg_cache = self.bind_groups.write().unwrap();
        ...
        for (i, step) in self.steps.iter().enumerate() {
            ...
            match step {
                Step::Fused {
                    ...
                    let env_data: [u32; 4] = [
                        *env.get("S0").unwrap_or(&1) as u32,
                        *env.get("S1").unwrap_or(&1) as u32,
                        *env.get("S2").unwrap_or(&1) as u32,
                        *env.get("S3").unwrap_or(&1) as u32,
                    ];
                    ...
                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        ...
                        for (bind_idx, &in_id) in input_ids.iter().enumerate() {
                            ...
                            entries.push(wgpu::BindGroupEntry {
                                binding: (bind_idx + 1) as u32,
                                resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
                                    buffer: unsafe { &*Arc::as_ptr(&b) },
                                    offset: 0,
                                    size: None,
                                }),
                            });
                        ...
                        entries.push(wgpu::BindGroupEntry {
                            binding: (input_ids.len() + 1) as u32,
                            resource: wgpu::BindingResource::Buffer(wgpu::BufferBinding {
                                buffer: unsafe { &*Arc::as_ptr(&out_b) },
                                offset: 0,
                                size: None,
                            }),
                        });
                ...
                Step::MatMul {
                    ...
                    let m_real = m.eval(env) as u32;
                    let k_real = k.eval(env) as u32;
                    let n_real = n.eval(env) as u32;
                    let dims = [m_real, k_real, n_real];
                    queue.write_buffer(uniform_buf, 0, bytemuck::cast_slice(&dims));

                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let a_b = get_buf(*a_id);
                        let b_b = get_buf(*b_id);
                ...
                Step::Transpose {
                    ...
                    let r_real = rows.eval(env) as u32;
                    let c_real = cols.eval(env) as u32;
                    let dims = [r_real, c_real, 0, 0];
                    queue.write_buffer(uniform_buf, 0, bytemuck::cast_slice(&dims));
                ...
                Step::Embedding {
                    ...
                    let h_real = hidden_size.eval(env) as u32;
                    let dims = [seq_real, h_real, 0, 0];
                    queue.write_buffer(uniform_buf, 0, bytemuck::cast_slice(&dims));

                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let w_b = get_buf(*w_id);
                        let idx_b = get_buf(*idx_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                label: None,
                                ...
                                entries: &[
                                    ...
                                    wgpu::BindGroupEntry {
                                        ...
                                        resource: wgpu::BindingResource::Buffer(
                                            wgpu::BufferBinding {
                                                buffer: unsafe { &*Arc::as_ptr(&idx_b) },
                                                offset: 0,
                                                size: None,
                                            },
                                        ),
                ...
                Step::CrossEntropy {
                    ...
                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let l_b = get_buf(*l_id);
                        let t_b = get_buf(*t_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                label: None,
                    ...
                    let o_b = get_buf(*out_id);
                    let zero = [0.0f32];
                    queue.write_buffer(&o_b, 0, bytemuck::cast_slice(&zero));
                ...
                Step::SoftmaxGrad {
                    ...
                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let out_data_b = get_buf(*out_data_id);
                        let og_b = get_buf(*out_grad_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                ...
                                entries: &[
                                    ...
                                    wgpu::BindGroupEntry {
                                        ...
                                        resource: wgpu::BindingResource::Buffer(
                                            wgpu::BufferBinding {
                                                buffer: unsafe { &*Arc::as_ptr(&out_data_b) },
                                                offset: 0,
                                                size: None,
                                            },
                                        ),
                ...
                | Step::BCEWithLogits {
                    ...
                    let dims = [s_real, 0, 0, 0];
                    queue.write_buffer(uniform_buf, 0, bytemuck::cast_slice(&dims));

                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let p_b = get_buf(*p_id);
                        let t_b = get_buf(*t_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                ...
                Step::LayerNorm {
                    ...
                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let i_b = get_buf(*in_id);
                        let g_b = get_buf(*gamma_id);
                        let b_b = get_buf(*beta_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                label: None,
                ...
                Step::FlashAttention {
                    ...
                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let q_b = get_buf(*q_id);
                        let k_b = get_buf(*k_id);
                        let v_b = get_buf(*v_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                label: None,
                ...
                Step::HorizontalFusionGroup {
                    matmuls,
                    uniform_bufs,
                } => {
                    let bind_groups = bg_cache.entry(i).or_insert_with(|| {
                        let mut bgs = Vec::new();
                        for (group_idx, (a_id, b_id, out_id, _, _, _)) in matmuls.iter().enumerate()
                        {
                            let a_b = get_buf(*a_id);
                            let b_b = get_buf(*b_id);
                            let o_b = get_buf(*out_id);
                            bgs.push(Arc::new(device.create_bind_group(
                                &wgpu::BindGroupDescriptor {
                                    label: None,
                                    layout: &pipeline.get_bind_group_layout(0),
                                    entries: &[
                                        wgpu::BindGroupEntry {
                                            binding: 0,
                                            resource: uniform_bufs[group_idx].as_entire_binding(),
                                        },
                                        wgpu::BindGroupEntry {
                                            binding: 1,
                                            resource: wgpu::BindingResource::Buffer(
                    ...
                    cpass.set_pipeline(pipeline);
                    for (group_idx, (_, _, _, m, k, n)) in matmuls.iter().enumerate() {
                        let m_real = m.eval(env) as u32;
                        let k_real = k.eval(env) as u32;
                        let n_real = n.eval(env) as u32;
                        let dims = [m_real, k_real, n_real];
                        queue.write_buffer(
                            &uniform_bufs[group_idx],
                            0,
                            bytemuck::cast_slice(&dims),
                        );
                        cpass.set_bind_group(0, &*bind_groups[group_idx], &[]);
                        cpass.dispatch_workgroups(n_real.div_ceil(16), m_real.div_ceil(16), 1);
                    }
                }
                Step::MaxPool2d {
                ...
                | Step::AvgPool2d {
                    ...
                    let n_r = n.eval(env) as u32;
                    let c_r = c.eval(env) as u32;
                    let h_r = h.eval(env) as u32;
                    let w_r = w.eval(env) as u32;
                    let out_h = h_r / *k;
                    let out_w = w_r / *k;
                ...
                Step::BatchNorm {
                    ...
                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let x_b = get_buf(*x_id);
                        let g_b = get_buf(*g_id);
                        let b_b = get_buf(*b_id);
                        let rm_b = get_buf(*rm_id);
                        let rv_b = get_buf(*rv_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                label: None,
                ...
                Step::Cond {
                    ...
                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let c_b = get_buf(*c_id);
                        let t_b = get_buf(*t_id);
                        let f_b = get_buf(*f_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                label: None,
                ...
                Step::WhileLoop {
                    ...
                    let s_real = size.eval(env) as u32;
                    let dims = [s_real, *max_iters as u32, 0, 0];
                    queue.write_buffer(uniform_buf, 0, bytemuck::cast_slice(&dims));

                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let s_b = get_buf(*s_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                label: None,
                ...
                Step::TopK {
                    ...
                    let r_real = rows.eval(env) as u32;
                    let c_real = cols.eval(env) as u32;
                    let dims = [r_real, c_real, *k as u32, 0];
                    queue.write_buffer(uniform_buf, 0, bytemuck::cast_slice(&dims));

                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        let i_b = get_buf(*in_id);
                        let v_b = get_buf(*val_out_id);
                        let idx_b = get_buf(*idx_out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                label: None,
                                layout: &pipeline.get_bind_group_layout(0),
                    ...
                    cpass.set_pipeline(pipeline);
                    cpass.set_bind_group(0, &*bind_group[0], &[]);
                    cpass.dispatch_workgroups(r_real.div_ceil(64), 1, 1);
                }
                Step::PagedAttention {
                    q_id,
                    k_id,
                    ...
                    let s_real = size.eval(env) as u32;
                    // Provide the WGSL shader with dynamic metadata
                    let dims = [
                        s_real,
                        *env.get("S0").unwrap_or(&1) as u32,
                        *head_dim,
                    ...
                    let bind_group = bg_cache.entry(i).or_insert_with(|| {
                        ...
                        let k_b = get_buf(*k_id);
                        let v_b = get_buf(*v_id);
                        let kv_b = get_buf(*kv_id);
                        let bt_b = get_buf(*bt_id);
                        let cl_b = get_buf(*cl_id);
                        let o_b = get_buf(*out_id);
                        vec![Arc::new(device.create_bind_group(
                            &wgpu::BindGroupDescriptor {
                                label: None,
...
/// Compiles the captured trace into an optimized `CompiledModel` with generated WGSL shaders.
pub fn compile<F>(
    device: &wgpu::Device,
    mut model_fn: F,
    dummy_inputs: &[&TensorNode<LazyBackend>],
) -> CompiledModel
where
    F: FnMut(&[&TensorNode<LazyBackend>]) -> TensorNode<LazyBackend>,
{
        ...
        match step {
            ...
            Step::FlashAttention { head_dim, .. } => {
                let code = format!("
                    ...
                    // Software Exponential Approximation
                    ...
                    fn main(@builtin(global_invocation_id) id: vec3<u32>, @builtin(local_invocation_id) local_id: vec3<u32>) {{
                        ...
                        for (var t = 0u; t < d.seq_len; t = t + 32u) {{
                            ...
                            for (var c = 0u; c < 32u; c = c + 1u) {{
                                ...
                                // FLASH-ATTENTION 2 Causal Masking
            ...
            Step::TopK { .. } => {
                let code = "
                    struct Dims { rows: u32, cols: u32, k: u32, pad: u32 }
                    @group(0) @binding(0) var<uniform> d: Dims;
                    @group(0) @binding(1) var<storage, read> input: array<f32>;
                    @group(0) @binding(2) var<storage, read_write> out_vals: array<f32>;
                    @group(0) @binding(3) var<storage, read_write> out_idxs: array<f32>;
                    
                    @compute @workgroup_size(64, 1, 1)
                    fn main(@builtin(global_invocation_id) id: vec3<u32>) {
                        let row = id.x; if (row >= d.rows) { return; }
                        let start_in = row * d.cols; let start_out = row * d.k;
                        
                        for (var iter = 0u; iter < d.k; iter = iter + 1u) {
                            var max_val = -1000000.0; var max_idx = 0u;
                            for (var i = 0u; i < d.cols; i = i + 1u) {
                                let v = input[start_in + i];
                                var already_picked = false;
                                for (var j = 0u; j < iter; j = j + 1u) {
                                    if (u32(out_idxs[start_out + j]) == i) { already_picked = true; break; }
                                }
                                if (!already_picked && v > max_val) { max_val = v; max_idx = i; }
                            }
                            out_vals[start_out + iter] = max_val; out_idxs[start_out + iter] = f32(max_idx);
                        }
                    }
                ";
                let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
